

class YoutubeData {
  final String title;
  final String thumbNail;
  final String avatar;
  final String username;
  final String views;
  final String duration;
  final String ageOfVideo;

  YoutubeData(
      {this.title,
      this.thumbNail,
      this.avatar,
      this.username,
      this.views,
      this.duration,
      this.ageOfVideo});
}

List<YoutubeData> homePageData = [
  YoutubeData(
    title: 'Skip Bayless wouldn\'t be surprised to see Tom Brady walk away from the Patriots | NFL | UNDISPUTED',
    thumbNail: 'https://i.ytimg.com/vi/vmQwVR4-aaA/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a-/AAuE7mDJQ6_uPAa6V_8wxoYIsB03GYe0UwdD8u81qA=s68-c-k-c0x00ffffff-no-rj-mo',
    username: 'Skip and Shannon: UNDISPUTRD',
    views: '4.4k views',
    duration: '9:02',
    ageOfVideo: '44 minutes ago',
  ),
  YoutubeData(
    title: '\$2 Ramen VS \$100 Ramen in Tokyo, Japan!!! Never Seen Before!!',
    thumbNail: 'https://i.ytimg.com/vi/a5yP1yqGEMo/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a/AGF-l7892aySgGAoMdMasvTMDEDLPMl-gqKI4rIFHQ=s88-c-k-c0xffffffff-no-rj-mo',
    username: 'Best Ever Food Review Show',
    views: '2.4M views',
    duration: '16:59',
    ageOfVideo: '1 month ago',
  ),
  YoutubeData(
    title: 'Stephen Curry\'s Best Plays Of The Decade',
    thumbNail: 'https://i.ytimg.com/vi/P5_xgip67ac/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a/AGF-l78U5c2H6ecI8vrMu9seGbF3K3fHJ7rtaU8n=s88-c-k-c0xffffffff-no-rj-mo',
    username: 'NBA',
    views: '87k views',
    duration: '12:54',
    ageOfVideo: '4 hours ago',
  ),
  YoutubeData(
    title: 'Samsung Galaxy Fold Unboxing - \$2000 Smartphone! (Minecraft, Fortnite, PUBG, Call of Duty Mobile)',
    thumbNail: 'https://i.ytimg.com/vi/2_vJlKjSTqQ/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a-/AAuE7mCILxSGi7htrKykTFxguMfegFQHOapdjoqd2g=s68-c-k-c0x00ffffff-no-rj-mo',
    username: 'TheRelaxingEnd',
    views: '2.9M views',
    duration: '11:46',
    ageOfVideo: '2 months ago',
  ),
  YoutubeData(
    title: 'Shin Lim Shocks Ellen with a Magic Trick Proving Their Similarities',
    thumbNail: 'https://i.ytimg.com/vi/h0HdOFc9e4k/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a/AGF-l7-vAW-s0xZI7oURqwN9_hHXlEU9W0Fy2lYtqw=s88-c-k-c0xffffffff-no-rj-mo',
    username: 'TheEllenShow',
    views: '2.3M views',
    duration: '4:38',
    ageOfVideo: '2 weeks ago',
  ),
  YoutubeData(
    title: 'Jackie Chan Roasting Everyone EVEN HIMSELF!',
    thumbNail: 'https://i.ytimg.com/vi/Tf90zyPefkU/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a/AGF-l7926Yx7SlRKhOLOAKed6SaADsm3WpQzr38LCQ=s88-c-k-c0xffffffff-no-rj-mo',
    username: 'Overcast',
    views: '2.7M views',
    duration: '6:39',
    ageOfVideo: '10 months ago',
  ),
];

List<YoutubeData> trendingPageData = [
  YoutubeData(
    title: 'Justin Bieber - Yummy(Official Video)',
    thumbNail: 'https://i.ytimg.com/vi/8EJ3zbKTWQ8/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a-/AAuE7mD2KkT_7ftpRJpH_n6ZNeSJFvFlgRGqNcoOzq4q=s88-c-k-c0xffffffff-no-nd-rj',
    username: 'Justin Bieber',
    views: '3.5M views',
    duration: '3:51',
    ageOfVideo: '9 hours ago',
  ),
  YoutubeData(
    title: '‘We’ll eliminate you’: Donald Trump says ‘we terminated Iran general to stop a war not start one’',
    thumbNail: 'https://i.ytimg.com/vi/O7Yj9pnOUP8/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a/AGF-l7-kDB0tGN8MdaCfS_pr8fNnyG0UTyDd_rWOrQ=s88-c-k-c0xffffffff-no-rj-mo',
    username: 'The Sun',
    views: '1.6M views',
    duration: '4:26',
    ageOfVideo: '1 day ago',
  ),
  YoutubeData(
    title: 'When Roddy Ricch was recording "The Box"',
    thumbNail: 'https://i.ytimg.com/vi/TGZgXgPnflM/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a/AGF-l791VS4taaYXbsadgkZB8bRusqJKMS2I4qv86w=s88-c-k-c0xffffffff-no-rj-mo',
    username: 'Lenarr Young',
    views: '702K views',
    duration:'1:33',
    ageOfVideo: '21 hours ago',
  ),
];

List<YoutubeData> subscriptionsPageData = [
  YoutubeData(
    title: 'Video captures flying objects that official can\'t explain',
    thumbNail: 'https://i.ytimg.com/vi/JfQ51ESeXj0/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a/AGF-l79d7bv3lD8Hf5Cu3jBLyekhBnG9tDQt3we-LA=s88-c-k-c0xffffffff-no-rj-mo',
    username: 'CNN',
    views: '2K views',
    duration: '2:12',
    ageOfVideo: '18 minutes ago',
  ),
  YoutubeData(
    title: 'Jerry Jones must give up some power if the Cowboys want to win a Super Bowl | NFL Countdown',
    thumbNail: 'https://i.ytimg.com/vi/VWWiELD9R-0/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a/AGF-l7_sFD-R_7HD4eVRrtFp4flC2vUKSv9pf3wX_g=s88-c-k-c0xffffffff-no-rj-mo',
    username: 'ESPN',
    views: '2.2K views',
    duration: '6:33',
    ageOfVideo: '1 hours ago',
  ),
  YoutubeData(
    title: 'Tom Brady had a sense of relief after Patriots’ Wild Card loss – Seth Wickersham | NFL on ESPN',
    thumbNail: 'https://i.ytimg.com/vi/megIJMXLe3M/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a/AGF-l7_sFD-R_7HD4eVRrtFp4flC2vUKSv9pf3wX_g=s88-c-k-c0xffffffff-no-rj-mo',
    username: 'ESPN',
    views: '29K views',
    duration: '4:17',
    ageOfVideo: '4 hours ago',
  )
];

List<YoutubeData> inboxPageData = [
  YoutubeData(
    title: 'Stephen A. shocked to hear of Cowboys hiring Mike McCarthy live on air | First Take',
    thumbNail: 'https://i.ytimg.com/vi/oNtD5N9SlCg/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a/AGF-l7_sFD-R_7HD4eVRrtFp4flC2vUKSv9pf3wX_g=s88-c-k-c0xffffffff-no-rj-mo',
    username: 'EPSN',
    ageOfVideo: '2 days ago',
  ),
  YoutubeData(
    title: 'Woman Fears Daughter Allows Child to Talk to dad who Almost Killed Her - Daughter Denies Allegation',
    thumbNail: 'https://i.ytimg.com/vi/e6ns1eW7R04/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a/AGF-l79JlRdgRZZccpVkxyZIjXuVRvjctvpvfafHUQ=s176-c-k-c0x00ffffff-no-rj-mo',
    username: 'Dr.Phil',
    ageOfVideo: '16 hours ago',
  ),
  YoutubeData(
    title: '5 Simple Snacks Without Added Sugar • Tasty',
    thumbNail: 'https://i.ytimg.com/vi/IWu3XKKsT7U/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a/AGF-l79wniZfBdNgKPqOAmbEOGvIcEZmxBCOYfsDcw=s88-c-k-c0xffffffff-no-rj-mo',
    username: 'Tasty',
    ageOfVideo: '3 hours ago',
  ),
  YoutubeData(
    title: 'Luffy\'s Future In The Story! - One Piece Discussion',
    thumbNail: 'https://i.ytimg.com/vi/KNNsUGlJKP8/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a/AGF-l78QspTzyrdA0pQ5iW3rC1R_jHnCda-t6WwlPw=s88-c-k-c0xffffffff-no-rj-mo',
    username: 'Tekking101',
    ageOfVideo: '7 hours ago',
  ),
  YoutubeData(
    title: 'HUGE FISH FILLETING AND CUTTING DAY',
    thumbNail: 'https://i.ytimg.com/vi/eSY_4A1ARak/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a/AGF-l7_8Qfuv4-QC_zZo4mRDDFAEmQLNKYujSYdBFA=s88-c-k-c0xffffffff-no-rj-mo',
    username: 'Morigami',
    ageOfVideo: '14 hours ago',
  ),
  YoutubeData(
    title: 'How Hackers Can Jam Your Wi-Fi Networks',
    thumbNail: 'https://i.ytimg.com/vi/T3Mg221FmdU/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a/AGF-l79JBx7ha9vUVJamAdb0_ahAW4mOfAynm2G4CQ=s88-c-k-c0xffffffff-no-rj-mo',
    username: 'Null Byte',
    ageOfVideo: '1 day ago',
  ),
  YoutubeData(
    title: 'How 2020 candidates see their path to victory, starting in Iowa',
    thumbNail: 'https://i.ytimg.com/vi/QTzbML8Lm-Y/hqdefault.jpg',
    avatar: 'https://yt3.ggpht.com/a/AGF-l794wyrf-6ID9c8sSOQRoaj8eS6EkmycceeW1g=s88-c-k-c0xffffffff-no-rj-mo',
    username: 'CBS News',
    ageOfVideo: '1 day ago',
  )
];